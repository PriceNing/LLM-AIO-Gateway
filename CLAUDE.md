...
tests/                          # pytest + FastAPI TestClient (278 test cases)
```
...
- `config.json` changes trigger auto-reload (uvicorn `reload_includes`).
...
- **Tool calls serialization:** liteLLM streaming returns `ChatCompletionDeltaToolCall` Pydantic objects in `delta.tool_calls`. These must be converted to plain dicts via `tc.model_dump(exclude_none=True)` before `json.dumps()`. Never assign liteLLM/openai types directly into dicts that will be JSON-serialized. Applies to `_stream_chat()`, non-streaming `chat_completions()`, and `_stream_anthropic_messages()` in proxy.py.
